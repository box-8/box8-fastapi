...
def set_secure_cookie(response: Response, key: str, value: str, max_age: int = None) -> None:
    """Configure un cookie avec les paramètres de sécurité appropriés"""
    cookie_domain = os.getenv("COOKIE_DOMAIN", None)
    print(f"Setting cookie {key} with domain: {cookie_domain}")
    ...
    response.set_cookie(
        ...
        samesite="none",  # Permettre les requêtes cross-origin
        secure=True,  # Nécessaire pour HTTPS et SameSite=None
        domain=cookie_domain,
        path="/"  # S'assurer que le cookie est disponible pour tout le site
    )
...
def set_session_cookie(response: Response, access_token: str) -> None:
    ...
    set_secure_cookie(response, "session", access_token)
