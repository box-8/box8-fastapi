...
from app.auth.auth import get_current_user, create_access_token, ACCESS_TOKEN_EXPIRE_MINUTES, set_session_cookie, set_secure_cookie
...
origins = [
    ...
    "https://box7-react-c6836a475b72.herokuapp.com"  # URL de production
]
...
async def extend_session_middleware(request: Request, call_next):
    ...
    if session:
        ...
        print(f"[Middleware] No session cookie found")
...
@app.post("/designer/upload-diagram")
async def upload_diagram(request: Request, file: UploadFile = File(...)):
    try:
        ...
        # Sauvegarder le fichier dans le dossier designer
        designer_folder = get_absolute_path("sharepoint/designer")
        if not os.path.exists(designer_folder):
            os.makedirs(designer_folder)
            
        file_path = os.path.join(designer_folder, file.filename)
        async with aiofiles.open(file_path, 'wb') as f:
            await f.write(content)
            
        return {"message": "Diagramme uploadé avec succès", "filename": file.filename}
...
async def set_llm(request: Request, llm_data: LLMSelection):
    ...
    set_secure_cookie(response, "selected_llm", llm_data.llm)
    return response
